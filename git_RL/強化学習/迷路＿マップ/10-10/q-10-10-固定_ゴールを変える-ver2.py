...
# 行は状態0～7、列は移動方向で↑、→、↓、←を表す
theta_2 = np.array([[np.nan, 1, 1, np.nan],  # s0 右下
                    [np.nan, 1, np.nan, 1],  # s1 左右
                    [np.nan, 1, 1, 1],  # s2 左右下
                    [ np.nan,np.nan,np.nan,1],  # s3 左
...
pi_2 = simple_convert_into_pi_from_theta(theta_2)
...
# Q学習で迷路を解く関数の定義2、状態と行動の履歴および更新したQを出力


def goal_maze_ret_s_a_Q_2(Q, epsilon, eta, gamma, pi):
    s = 0  # スタート地点
    a = a_next = get_action(s, Q, epsilon, pi)  # 初期の行動
    s_a_history = [[0, np.nan]]  # エージェントの移動を記録するリスト
...
for i in range(1):#(X)
    ...
    while is_continue:  # is_continueがFalseになるまで繰り返す
        ...
        if episode < 200:
            ...
            [s_a_history, Q] = goal_maze_ret_s_a_Q_2(Q, epsilon, eta, gamma, pi_2)
            print('11111111111111111',episode)
        # 状態価値の変化
        ...
        print("迷路を解くのにかかったステップ数は" + str(len(s_a_history) - 1) + "です",episode)
        print(s_a_history)
